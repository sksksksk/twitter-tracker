 ...
 */
class TwitterTracker_Tweet_Widget extends TwitterTracker_SW_Widget {

    /** constructor */
    function TwitterTracker_Tweet_Widget() {
		$name = __( 'Twitter Tweet Tracker', 'twitter-tracker' );
		$options = array( 'description' => __( 'A widget which displays a single Tweet.', 'twitter-tracker' ) );
        $this->WP_Widget( 'twitter-tweet-tracker', $name, $options );	
    }
    ...
    function update($new_instance, $old_instance) {
		...
		delete_option( 'widget_config_twitter-tracker-2' );
    ...
    function form( $instance ) {
		...
		$twitter_tweet = isset( $twitter_tweet ) ? $twitter_tweet : '';
		...
		$old_options = get_option( 'widget_config_twitter-tracker-2' );
		if ( ! empty( $old_options ) ) {
			...
			$this->show_old_option( $old_options[ 'twitter_tweet' ], 'Twitter Tweet' );
		...
		$tweet_note = __( 'Tweet to show.', 'twitter-tracker' );
		$this->input_text( __( 'Twitter tweet:', 'twitter-tracker' ), 'twitter_tweet', $twitter_tweet, $tweet_note );
	...
	{
		$classes = "widget_twitter-tweet-tracker " . esc_attr( $class ) . " ";
		return str_replace( 'widget_twitter-tweet-tracker', $classes, $before_widget );
	}
